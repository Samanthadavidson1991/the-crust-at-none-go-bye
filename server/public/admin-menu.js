...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let selectedMasterToppings = [];
    let selectedMasterToppingPrices = {};

    // Create master toppings modal
    const masterToppingsModalBg = document.createElement('div');
    masterToppingsModalBg.style.position = 'fixed';
    masterToppingsModalBg.style.top = '0';
    masterToppingsModalBg.style.left = '0';
    masterToppingsModalBg.style.width = '100vw';
    masterToppingsModalBg.style.height = '100vh';
    masterToppingsModalBg.style.background = 'rgba(0,0,0,0.5)';
    masterToppingsModalBg.style.display = 'none';
    masterToppingsModalBg.style.justifyContent = 'center';
    masterToppingsModalBg.style.alignItems = 'center';
    masterToppingsModalBg.style.zIndex = '2000';
    document.body.appendChild(masterToppingsModalBg);

    const masterToppingsModal = document.createElement('div');
    masterToppingsModal.style.background = '#fff';
    masterToppingsModal.style.padding = '24px';
    masterToppingsModal.style.borderRadius = '8px';
    masterToppingsModal.style.boxShadow = '0 2px 16px rgba(0,0,0,0.2)';
    masterToppingsModal.style.minWidth = '320px';
    masterToppingsModal.style.border = '3px solid green';
    masterToppingsModalBg.appendChild(masterToppingsModal);

    function showMasterToppingsModal() {
        let html = `<h3>Select Master Toppings</h3>`;
        html += `<form id='master-toppings-select-form'>`;
        masterToppings.forEach(mt => {
            const key = mt.name + '|' + mt.category;
            html += `<label style='display:block;margin-bottom:6px;'>`;
            html += `<input type='checkbox' class='admin-master-topping-checkbox' data-name='${mt.name}' data-category='${mt.category}' ${selectedMasterToppings.includes(key) ? 'checked' : ''}> ${mt.name} (${mt.category})`;
            html += ` Price: <input type='number' class='admin-master-topping-price' data-name='${mt.name}' data-category='${mt.category}' value='${selectedMasterToppingPrices[key] || ''}' step='0.01' min='0' style='width:60px;'>`;
            html += `</label>`;
        });
        html += `</form>`;
        html += `<button id='master-toppings-modal-save'>Save</button> <button id='master-toppings-modal-cancel'>Cancel</button>`;
        masterToppingsModal.innerHTML = html;
        masterToppingsModalBg.style.display = 'flex';

        masterToppingsModal.querySelector('#master-toppings-modal-cancel').onclick = function() {
            masterToppingsModalBg.style.display = 'none';
        };
        masterToppingsModal.querySelector('#master-toppings-modal-save').onclick = function() {
            // Save selected toppings and prices
            selectedMasterToppings = [];
            selectedMasterToppingPrices = {};
            masterToppingsModal.querySelectorAll('.admin-master-topping-checkbox').forEach(cb => {
                if (cb.checked) {
                    const name = cb.getAttribute('data-name');
                    const category = cb.getAttribute('data-category');
                    const key = name + '|' + category;
                    selectedMasterToppings.push(key);
                    const priceInput = masterToppingsModal.querySelector(`.admin-master-topping-price[data-name='${name}'][data-category='${category}']`);
                    let price = parseFloat(priceInput.value);
                    if (!isNaN(price)) selectedMasterToppingPrices[key] = price;
                }
            });
            masterToppingsModalBg.style.display = 'none';
        };
